...
final_grid_spacing  = config.get("final_grid_spacing", 5.0) 
...
def get_elastix_object(n_resolutions=4, 
                      max_iterations=2000, 
                      metric='AdvancedMattesMutualInformation', # AdvancedMattesMutualInformation, AdvancedMeanSquares
    ...
    # ======================================================================
    # PURE B-SPLINE REGISTRATION (LOCAL DEFORMATION ONLY)
    # ======================================================================
    bspline_parameter_map = parameter_object.GetDefaultParameterMap('bspline', n_resolutions)
    
    # Basic configuration
    bspline_parameter_map['Metric'] = [metric]
    bspline_parameter_map['NumberOfSpatialSamples'] = [str(num_spatial_samples)]
    
    # Iteration control - focus all iterations on B-spline
    bspline_iterations = [str(max_iterations) for _ in range(n_resolutions)]
    bspline_parameter_map['MaximumNumberOfIterations'] = bspline_iterations
    
    # Conservative step lengths for precise deformation
    bspline_parameter_map['MaximumStepLength'] = [str(initial_step_length * 0.5)]  # Reduced for stability
    bspline_parameter_map['MinimumStepLength'] = [str(min_step_length)]  
    
    # B-spline grid control (key for accuracy)
    bspline_parameter_map['FinalGridSpacingInPhysicalUnits'] = [str(final_grid_spacing)]
    
    # Aggressive grid refinement schedule
    schedule = [str(2**(n_resolutions - i)) for i in range(n_resolutions)]  # e.g., [16, 8, 4, 2] for 4 res
    bspline_parameter_map['GridSpacingSchedule'] = schedule
    
    # Strict convergence criteria
    bspline_parameter_map['MinimumMetricDifference'] = ['1e-6']  
    bspline_parameter_map['UseAdaptiveStepSizes'] = ['true']
    
    parameter_object.AddParameterMap(bspline_parameter_map)
    
    # Debugging outputs
    bspline_parameter_map['WriteIterationInfo'] = ['true']
    bspline_parameter_map['LogToConsole'] = ['true']
    
    print("Pure B-spline Parameter Configuration:")
